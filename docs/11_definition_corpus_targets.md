...
이 문서는 `hate_speech_authoritative` vector collection에 넣을 혐오표현 정의, 판단 기준, 보호 속성, 심각도 평가 기준 문서 후보를 정리한다. 내부 category taxonomy는 `hate_speech_taxonomy` collection에 별도로 적재한다.
...
1. 한국어 dataset annotation guideline과 label description
2. 국내 공식/준공식 혐오표현 문서
3. 국제 기준과 플랫폼 정책
4. 논문 초록, 라벨 체계 설명, 저자가 공개한 summary

## 내부 Seed 구성

다음 내부 문서는 `hate_speech_taxonomy`에 반드시 포함한다.

| 우선순위 | source_id | 역할 | 수집 판단 |
| --- | --- | --- | --- |
| A | `internal_taxonomy_allowed_categories` | 허용 카테고리 목록 | 포함 |
| A | `internal_taxonomy_hate_threshold` | 공격 강도와 비혐오 경계 | 포함 |
...
| A | `internal_taxonomy_context_exception` | 인용·보도·풍자 문맥 예외 | 포함 |
| A | `internal_taxonomy_multi_label_rule` | 복수 선택과 중복 금지 | 포함 |
| A | `internal_taxonomy_hate_type` | 표현 방식 정규화 | 포함 |
| A | `internal_taxonomy_target_group` | 표적 명사구 정규화 | 포함 |
| A | `internal_category_cards` | 13개 category의 정의·포함·제외·경계·검색 cue | 포함 |
| A | `internal_conflict_rules` | 충돌 및 우선순위 규칙 | 포함 |

내부 seed는 규칙 card 10개와 category card 13개, 총 23개 문서다.
...
실제 혐오표현 사례는 `hate_speech_examples` collection의 대상이며, 이 문서의 `hate_speech_authoritative` collection과 분리한다.
...
```text
definition-corpus-2026-07-16-v0.3
```
...
MVP에서는 내부 taxonomy card, 한국어 dataset guideline/label description, 국내 기준 3개, 국제 기준 3개, YouTube 정책 1개로 `definition-corpus-2026-07-16-v0.3`를 만든다.

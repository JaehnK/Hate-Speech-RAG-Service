...
- collection 이름은 `hate_speech_collection`을 기본값으로 둔다.
- embedding provider와 collection 이름은 `analysis_runs`에 저장한다.
...
1. Python 3.11을 사용할지
2. dependency 관리 도구를 `uv`로 둘지
3. raw 프로젝트 submodule 경로를 어디로 둘지
4. Chroma를 persistent directory로 시작할지
5. 관리자 인증을 `X-Admin-Token`으로 시작할지
6. server-side template로 웹 보고서를 먼저 만들지

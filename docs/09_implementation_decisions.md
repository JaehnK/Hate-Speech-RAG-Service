...
| 대상 | `legacy/YouTubeHateSpeech/`, `legacy/hateSpeechRAG/` |
...
- UNSMILE은 내부 기본 혐오 카테고리 평가에만 사용하고, 공개 또는 상업 retrieval corpus에는 포함하지 않는다.
...
- 초기 prompt version은 `category-rag-v0.1.0`이었고, example 본문/라벨 전달과 context instruction 격리는 `category-rag-v0.2.0`, 한국어 보고서용 분석 사유와 이를 검증하는 계약은 현재 `category-rag-v0.3.0`이다.
- output validation은 저장 전 필수로 수행한다.
- license tier가 `permission_required`인 corpus는 retrieval corpus에서 제외한다.

### DEC-020 사용자 신원 확인 방식

| 항목 | 값 |
| --- | --- |
| 상태 | 확정 |
| 결정 | Google OAuth Authorization Code flow(PKCE 포함)만 사용한다. |
| 근거 | 사용자 결정 사항. 자체 회원가입/비밀번호 로그인을 만들지 않아 인증 관련 공격면을 줄인다. |
| 영향 문서 | `00_project_brief.md`, `01_user_scenarios.md`, `02_hld.md`, `03_data_model.md`, `05_api_spec.md`, `30_auth_oauth_byok.md` |

정책:

- 사용자 신원의 기준은 `users.google_sub`다.
- Google 외 IdP, 이메일/비밀번호 로그인은 MVP 범위에 포함하지 않는다.
...
| --- | --- |
| 상태 | 확정 |
| 결정 | 서버가 발급하는 opaque 세션 토큰 + HttpOnly 쿠키. 클라이언트가 해석하는 JWT를 세션의 기준으로 사용하지 않는다. |
| 근거 | 즉시 revoke 가능해야 하고(JWT는 서버 상태 없이 무효화가 어려움), 쿠키 값을 프론트엔드 JS가 다룰 필요가 없다. |
| 영향 문서 | `02_hld.md`, `03_data_model.md`, `05_api_spec.md`, `07_backend_design.md`, `30_auth_oauth_byok.md` |

정책:

- 세션은 `user_sessions` 테이블에 저장하고, DB에는 토큰 해시만 남긴다.
- 세션 저장소는 MVP에서 PostgreSQL을 사용한다(Redis 등 분리는 보류 사항).
- 쿠키 속성(`HttpOnly`, `Secure`, `SameSite=Lax`)은 `30_auth_oauth_byok.md`를 따른다.

### DEC-022 사용자 API 키(BYOK) 관리 방식

| 항목 | 값 |
| --- | --- |
| 상태 | 확정 |
| 결정 | Anthropic API 키와 Upstage API 키는 사용자가 직접 등록(BYOK)하고, 서버는 Fernet 암호화 후 저장한다. YouTube API 키는 계속 운영자 공용 키를 사용한다. |
| 근거 | 사용자 결정 사항. 공개 배포 시 LLM/embedding 비용이 운영자에게 무제한으로 발생하는 것을 막기 위함이다. YouTube Data API는 무료 쿼터 기반이라 운영자가 계속 부담 가능하다. |
| 영향 문서 | `00_project_brief.md`, `01_user_scenarios.md`, `02_hld.md`, `03_data_model.md`, `05_api_spec.md`, `07_backend_design.md`, `08_mvp_plan.md`, `30_auth_oauth_byok.md` |

정책:

- `user_api_keys.encrypted_key`는 애플리케이션 레벨 Fernet 암호화 후에만 저장한다.
- 마스터 키(`API_KEY_ENCRYPTION_KEY`)는 환경변수로만 관리하고 저장소에 커밋하지 않는다.
- 키 원문은 worker의 job 실행 스코프 밖으로 나가지 않는다.
- 키 등록 시 provider 최소 비용 검증 호출을 수행한다.
...
10. Anthropic/Upstage 키 검증 호출로 어떤 최소 비용 endpoint를 사용할지
11. 세션 저장소를 PostgreSQL로 유지할지, 초기부터 Redis 등으로 분리할지
12. `API_KEY_ENCRYPTION_KEY`를 배포 환경(Railway 등)에 어떻게 안전하게 등록할지
...
| 인증, 세션, BYOK | `30_auth_oauth_byok.md` |
...
- 인증, 세션, BYOK 관련 결정이 관련 설계 문서와 `30_auth_oauth_byok.md`에 일관되게 반영되어 있다.

...
- 현재 댓글·자막 item은 같은 step 안에서 순차 실행한다.
...
- 현재 item 분류는 순차 실행한다. 진행 counter와 jobs API contract는 병렬화 뒤에도 유지하되, 중복 item 완료를 막기 위해 병렬 모드에서는 저장된 결과 row로 counter를 재계산한다.

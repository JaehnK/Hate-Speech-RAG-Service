...
- 기본값은 댓글·자막 item을 같은 step 안에서 순차 실행하며 `RAG_EXECUTION_MODE=parallel`일 때만 bounded executor를 사용한다.
- 향후 병렬 처리는 job을 점유한 worker 내부의 `analyze_comments`, `analyze_script`에만 적용한다. 다른 step과 여러 worker 사이에는 RAG 작업을 분산하지 않는다.
- 동일 query embedding을 두 vector collection에 재사용하고 외부 client 연결을 재사용한 뒤, 결과 unique constraint를 item checkpoint로 사용해 제한된 동시성을 활성화한다. 상세 순서는 `docs/22_rag_parallel_processing_plan.md`를 따른다.
...
- 문장 종결부호가 확인되면 그 경계를 우선하고, 없으면 45초·800자 상한으로 안전하게 분할한다.
...
- item 결과 insert와 전체/완료/성공/실패 counter를 같은 짧은 transaction으로 갱신한다.
- step 시작·종료에는 저장된 결과 row로 counter를 재조정한다.
...
- item 분류는 `RAG_EXECUTION_MODE`에 따라 순차 또는 제한된 병렬로 실행한다. 두 모드 모두 같은 result checkpoint와 jobs API contract를 사용하고 저장된 결과 row로 counter를 재조정한다.
- Upstage와 Anthropic 호출은 별도 semaphore를 사용하며 429의 `Retry-After`, 일시적 5xx와 timeout만 제한적으로 재시도한다.
- `rag_progress` operation log는 10건 또는 5초 단위로 완료/실패/in-flight, retry, token usage를 기록한다.
- 병렬 처리의 구현 순서와 성공 기준은 `docs/22_rag_parallel_processing_plan.md`를 따른다.

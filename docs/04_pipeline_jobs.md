...
- 향후 item 병렬 처리는 `docs/22_rag_parallel_processing_plan.md`의 ledger, lease, 멱등 저장을 먼저 적용한 뒤 제한된 동시성으로 활성화한다.
...
- 현재 item 분류는 순차 실행한다. 진행 counter와 jobs API contract는 병렬화 뒤에도 유지하되, 중복 item 완료를 막기 위해 병렬 모드에서는 item ledger의 terminal 상태로 counter를 재계산한다.

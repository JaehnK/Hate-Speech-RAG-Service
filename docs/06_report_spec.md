...
분석 이용자는 영상별 혐오표현 분포, 대표 사례, 스크립트 분석, 댓글 네트워크를 확인한다.

### 관리자

관리자는 보고서의 실패 섹션과 분석 설정 정보를 통해 수집, quota, LLM, vector store 문제를 추적한다.
...
- weight: 같은 작성자 쌍의 대댓글 수 또는 개별 대댓글 1건
...
| `methodology` | 모델, vector store, prompt version |

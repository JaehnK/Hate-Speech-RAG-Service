...
- coordinator는 `session_factory`로 짧은 transaction을 열어 결과 row 하나를 insert한다.
...
각 결과 저장 뒤 집계를 갱신하므로 out-of-order 완료에도 값이 역행하거나 전체 수를 넘지 않는다. coordinator는 item 완료 여부와 별개로 주기적으로 `job_steps.heartbeat_at`을 갱신한다. 모든 provider 호출이 오래 걸리는 상황도 active job으로 인식해야 하기 때문이다.

### 5. 제한된 동시성과 backpressure
...
- 댓글과 자막의 결과 payload, provider 호출 수, jobs API 진행률을 snapshot으로 고정한다.
...
### Phase P1. Item별 저장과 재시작 checkpoint

작업:

- classifier 결과를 ORM과 무관한 DTO로 반환하게 한다.
- item 하나의 결과를 별도 짧은 transaction으로 저장한다.
- 기존 결과가 있는 source item을 건너뛰는 resume query를 구현한다.
- 진행률을 결과 row 기반 projection으로 전환한다.
...
### Phase P2. Job 내부 bounded parallelism

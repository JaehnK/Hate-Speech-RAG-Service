...
def build_reports_router(get_session: Callable[[], Iterator[Session]], storage_dir: str) -> APIRouter:
    ...
    def get_network(report_id: UUID, session: SessionDependency) -> dict[str, Any]:
        report = _report(session, report_id)
    ...
    def create_export(report_id: UUID, request: ExportRequest, session: SessionDependency) -> dict[str, Any]:
        report = _report(session, report_id)
        export = export_service.create(session, report, request.format)
        return {
...
def build_report_pages_router(get_session: Callable[[], Iterator[Session]]) -> APIRouter:
    router = APIRouter(tags=["report-pages"])
    SessionDependency = Annotated[Session, Depends(get_session)]
    renderer = HtmlReportRenderer()

    @router.get("/reports/{report_id}", response_class=HTMLResponse)
    def report_page(report_id: UUID, session: SessionDependency) -> str:
        return renderer.render(_report(session, report_id))
